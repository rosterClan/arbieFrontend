...
const Next2GoTile = (props) => {
  ...
  const remove_tile = () => {
    let subject_tile = document.getElementsByClassName(key)[0];
    subject_tile.style.display = "none";
  }

  return (
    <div class={'next_2_tile_container ' + props.race_data['Start_time']}>
      ...
      <div class='next_2_tile_details'>
        ...
        <div class='next_2_gp_ele time'>
          <CounterTime trigger_function={remove_tile} target_time={new Date(props.race_data['Start_time'])}/>
        </div>
      </div>
    </div>
  );
